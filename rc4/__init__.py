class RC4:
    ...
    def crypt(self, data):
        ...
        return bytes([a ^ b for a, b in
                          zip(self._keystream(len(data)), data)])
...
if __name__ == "__main__":
 ...
 DEC 4096 HEX 1000:  37 0b 1c 1f  e6 55 91 6d   97 fd 0d 47  ca 1d 72 b8"""]
    ...
    def check(key, expected):
        print("Testing key: " + key.hex())
        ciphertext = RC4(key).crypt(bytearray(0x1010))
        
        cipher = RC4(key)
        ciphertext2 = b""
        for _ in range(0, 0x1010, 0x10): # check that splitting into chunks works
            ciphertext2 += cipher.crypt(bytearray(0x10))
        
        assert(ciphertext == ciphertext2)
        
        expected = [bytes.fromhex(line.split(":")[1]) for line in expected.split("\n")]
        for j, offset in enumerate(offsets):
            print("{:04d}: {}".format(offset, ciphertext[offset:offset+0x10].hex()))
